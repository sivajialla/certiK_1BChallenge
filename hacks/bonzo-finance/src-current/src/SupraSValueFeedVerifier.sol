...
contract SupraSValueFeedVerifier is Ownable2StepUpgradeable,UUPSUpgradeable {
    ...
    uint256 public blsPrecompileGasCost;
    ...
    event BLSPrecompileGasCostUpdated(uint256 previousGasCost, uint256 newGasCost);
    ...
    /// @notice to update the BLS precompile gas cost
    /// @dev only owner can update the BLS precompile gas cost
    /// @param _blsPrecompileGasCost new gas cost for BLS precompile
    function updateBLSPrecompileGasCost(uint256 _blsPrecompileGasCost) external onlyOwner {
        if (_blsPrecompileGasCost == 0) {
            revert InvalidOperation();
        }
        uint256 previousCost = blsPrecompileGasCost;
        blsPrecompileGasCost = _blsPrecompileGasCost;
        emit BLSPrecompileGasCostUpdated(previousCost, _blsPrecompileGasCost);
    }
}
